...
public:
    ...
    void Filewrite(std::string raw_text, bool clear = false);
    ...
    std::vector<std::filesystem::path> filespath; // filling is in folderfiles_iterator
    ...
    DPAR(std::string fullpath);

    //----------------------------------

};

class DPAR_S { //DPAR_STATIC FUNC

public:

    //A function for reading the contents of a file
    static std::string Fileread(std::string filepath);

    //A function for writing the contents to the file
    static void Filewrite(std::string filepath, std::string raw_text, bool clear = false);

    //A function to return a vector with all the objects in a folder
    static std::vector<std::filesystem::path> folderfiles_iterator(std::string folderpath);

    //A function to check if a file exists
    static bool file_validation(std::string filepath);

    //A function to check if a folder exists
    static bool folder_validation(std::string folderpath);
